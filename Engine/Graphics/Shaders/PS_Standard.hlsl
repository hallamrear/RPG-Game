...
float4 TriplanarSample(float3 worldPos, float3 worldNormal, float scale)
{
    ...
    float4 xProj = TextureList[TextureIndex].SampleLevel(linearSampler, worldPos.yz * scale, 0);
    float4 yProj = TextureList[TextureIndex].SampleLevel(linearSampler, worldPos.xz * scale, 0);
    float4 zProj = TextureList[TextureIndex].SampleLevel(linearSampler, worldPos.xy * scale, 0);
...
float4 main(VS_STANDARD_VERTEX_OUTPUT input) : SV_TARGET
{
    ...
    //{
        sampleColour = TextureList[TextureIndex].SampleLevel(linearSampler, input.UV, 0);
    //}
    ...
    return float4(sampleColour.rgb, 1.0f);
}